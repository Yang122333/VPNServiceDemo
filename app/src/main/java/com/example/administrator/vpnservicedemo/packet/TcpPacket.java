...
public class TcpPacket extends Packet {
    ...
    public TcpPacket(byte[] data, int offset, IPPacket ipPacket) {
        ...
        mIpInfo = ipPacket;
        //2byte
        mSourcePort = (data[offset] & 0xff << BYTE_LENGTH) | (data[offset + 1] & 0xff);
        mDestPort = (data[offset + TCP_DEST_PORT] & 0xff << BYTE_LENGTH) | (data[offset + TCP_DEST_PORT + 1] & 0xff);

        seq_number = (data[offset + TCP_SEQ] & 0xff) << (INT_LENGTH - BYTE_LENGTH) | (data[offset + TCP_SEQ + 1] & 0xff) << (INT_LENGTH - BYTE_LENGTH * 2) |
                (data[offset + TCP_SEQ + 2] & 0xff) << (INT_LENGTH - BYTE_LENGTH * 3) | (data[offset + TCP_SEQ + 3] & 0xff) << (INT_LENGTH - BYTE_LENGTH * 4);

        ack_number = (data[offset + TCP_ACK] & 0xff) << (INT_LENGTH - BYTE_LENGTH) | (data[offset + TCP_ACK + 1] & 0xff) << (INT_LENGTH - BYTE_LENGTH * 2) |
                (data[offset + TCP_ACK + 2] & 0xff) << (INT_LENGTH - BYTE_LENGTH * 3) | (data[offset + TCP_ACK + 3] & 0xff) << (INT_LENGTH - BYTE_LENGTH * 4);
        ...
        mWindowSize = ((data[offset + TCP_WINDOWSIZE] & 0xff) << BYTE_LENGTH) | (data[offset + TCP_WINDOWSIZE + 1] & 0xff);
    }
